...
import notify from '../../public/sound/notification.mp3';
...
const MessageContainer = ({ onBackUser }) => {
  ...
  useEffect(() => {
    const getMessages = async () => {
      setLoading(true);
      try {
        const res = await axiosInstance.get(`/api/message/${selectedConversation?._id}`);
        const data = res.data;
        if (!data.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSending(true);
    try {
      const res = await axiosInstance.post(`/api/message/send/${selectedConversation?._id}`, {
        messages: sendData,
      });
